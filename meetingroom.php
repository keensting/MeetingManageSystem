    ...
    <table class="table   table-striped">
        ...
        for($i=0;$i<$num;$i++) {
            ...
            <tr>
                ...
                <td>
                    <label class="btn btn-primary" onclick="getBoard(this.id);" id="<?php echo $i; ?>">详情</label>
                    <label class="btn btn-primary" onclick="modify(<?php echo $row->getId();  ?>);" >修改</label>
                    <?php
                    if($row->getState()==0) {
                                    ?> <label class="btn btn-danger" onclick = "delete_room(<?php echo $row->getId();  ?>);" > 删除</label >
...
<script>
    function modify(id)
    {
        ...
        document.location.href='./modifyroom.php?id='+id;
    }

    function delete_room(id)
    {
        var t=confirm('确认删除该会议室？');
        if(t==true)
        {
            sendRequest('./Ajax/responsPhp/ajax_del_room.php',tips,'id='+id);
        }
    ...
    function tips()
    {
        if(xmlHttp.readyState==4) {
            alert('该会议室已经删除！');
            document.location.href = './meetingroom.php';
        }
    }
</script>
</body>
